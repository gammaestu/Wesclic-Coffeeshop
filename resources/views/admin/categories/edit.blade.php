    ...
    <div class="bg-white rounded-xl shadow-md p-6 max-w-2xl">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                >
                    <option value="aktif" {{ old('status', $category->status) === 'aktif' ? 'selected' : '' }}>Active</option>
                    <option value="nonaktif" {{ old('status', $category->status) === 'nonaktif' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
