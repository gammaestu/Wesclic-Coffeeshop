...
@section('content')
    <div class="bg-white rounded-xl shadow-md p-6 max-w-2xl">
        <form action="{{ route('admin.menus.update', $menu) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            @if($menu->image)
            <div>
                <span class="block text-sm font-semibold text-[#3A3A3A] mb-2">Gambar saat ini</span>
                <img src="{{ $menu->image_url }}" alt="{{ $menu->name }}" class="h-24 w-24 object-contain rounded-lg border border-[#A3B18A]/30">
            </div>
            @endif
            <div>
                <label for="image_file" class="block text-sm font-semibold text-[#3A3A3A] mb-2">Ganti Gambar (PNG / JPG / SVG)</label>
                <input 
                    type="file" 
                    id="image_file" 
                    name="image_file" 
                ...
                >
                <p class="mt-1 text-xs text-[#3A3A3A]/70">Upload PNG, JPG, atau SVG (maks. 2MB). Kosongkan untuk tetap pakai gambar saat ini.</p>
                @error('image_file')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                >
                <p class="mt-1 text-xs text-[#3A3A3A]/70">Path manual jika tidak upload file</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
